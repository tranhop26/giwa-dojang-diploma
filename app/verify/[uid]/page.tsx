...
export default async function VerifyPage({ params }: VerifyPageProps) {
        ...
        <main className="flex-1 flex items-center justify-center p-6 bg-background relative">
          ...
          <Card className="w-full max-w-md border-border/40 bg-card/60 backdrop-blur-md shadow-2xl text-center p-6">
            <CardHeader className="space-y-2">
              ...
              <CardTitle className="font-display text-2xl">Attestation Not Found</CardTitle>
              <CardDescription>
                The transaction hash or UID did not match a valid diploma schema.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-muted-foreground">
                Please verify that the URL is correct or that the transaction has completed on-chain.
              </p>
            </CardContent>
  ...
  const formattedCompletionDate = new Date(data.completionDate * 1000).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  });

  const formattedIssueTime = new Date(Number(attestation.time) * 1000).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  });
          ...
          <div className="flex justify-center">
            ...
            {status === 'revoked' && (
              <Badge variant="destructive" className="px-4 py-1.5 text-xs font-semibold tracking-wider gap-1.5 shadow-lg">
                <ShieldAlert className="h-3.5 w-3.5" /> ✗ REVOKED
              </Badge>
            )}
            {status === 'expired' && (
          ...
          <Card className="border-border/40 bg-card/60 backdrop-blur-md shadow-2xl relative overflow-hidden">
            ...
            <CardContent className="px-6 md:px-10 text-center space-y-8 pb-10">
              <div className="space-y-2">
                <span className="text-xs text-muted-foreground uppercase tracking-widest">Proudly Presented To</span>
              ...
              <div className="space-y-2">
                <span className="text-xs text-muted-foreground uppercase tracking-widest">For Completing The Course</span>
              ...
              <div className="grid grid-cols-2 gap-6 pt-4 max-w-md mx-auto">
                <div className="space-y-1">
                  <span className="text-[10px] text-muted-foreground uppercase tracking-widest block">Completion Date</span>
                  <span className="text-sm font-semibold text-foreground">{formattedCompletionDate}</span>
                </div>
                <div className="space-y-1">
                  <span className="text-[10px] text-muted-foreground uppercase tracking-widest block">Verified Issuer</span>
              ...
              <div className="space-y-3.5 text-left text-xs text-muted-foreground max-w-lg mx-auto bg-black/20 p-4 rounded-xl border border-border/20">
                <div className="flex justify-between items-center gap-4">
                  <span className="font-medium shrink-0">Recipient Address:</span>
                ...
                <div className="flex justify-between items-center gap-4">
                  <span className="font-medium shrink-0">Issuer Address:</span>
                  <div className="flex items-center gap-1.5 overflow-hidden">
                ...
                <div className="flex justify-between items-center gap-4">
                  <span className="font-medium shrink-0">Issue Date:</span>
                  <span>{formattedIssueTime}</span>
                </div>

                <div className="flex justify-between items-center gap-4">
                  <span className="font-medium shrink-0">Attestation UID:</span>
                  <div className="flex items-center gap-1.5 overflow-hidden">
                ...
                {status === 'revoked' && (
                  <div className="flex justify-between items-center gap-4 text-destructive font-medium border-t border-destructive/10 pt-2 mt-2">
                    <span>Revoked Date:</span>
                    <span>{new Date(revocationTime * 1000).toLocaleDateString()}</span>
                  </div>
                )}
              </div>
            </CardContent>
